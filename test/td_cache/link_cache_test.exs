defmodule TdCache.LinkCacheTest do
  ...
  describe "LinkCache" do
    ...
    test "link_source_ids returns a map with linked entry count by source id" do
      ...
      assert LinkCache.link_source_ids("xxx", "yyy") == []
      assert LinkCache.link_source_ids("foo", "baz") == [42]
      assert LinkCache.link_source_ids("foo", "bar") == [42, 43, 44, 45]
      assert LinkCache.link_source_ids("bar", "foo") == [99, 123]
    end
  end
